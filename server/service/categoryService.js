...
class CategorService {
  ...
  async updateCategory(_id, { name, origin, detail }) {
    if (!name || !origin || !detail ) throw new Error("수정을 위한 데이터 정보가 일부 없습니다.");
    if (!_id) throw new Error("수정을 위한 카테고리 ID가 없습니다.");

    const updatedCategory = await Category.update(_id, { name, origin, detail });
